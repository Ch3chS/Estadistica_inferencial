...
```
En este caso las hipótesís son:
H0: Todos los algoritmos tienen el mismo tiempo de ejecución.
H1: Al menos uno de los algoritmos tiene un tiempo de ejecución que es distinto

Una prueba paramétrica que nos podría servir para resolver esto es el análisis anova

Para esto vamos a evaluar normalidad a continuación:

```{r prueba_normalidad_3}
shapiro.test(muestra_3_A)
shapiro.test(muestra_3_B)
shapiro.test(muestra_3_C)
```

Como se puede notar, si bien el p-value de los tiempos de B y C son lo suficientemente altos para que las muestras sea considerada normales, el p-value de los tiempos de A no lo es, por lo que no podemos asegurar que dichas muestras provienen de distribuciones cercanas a la normal, por lo que no usaremos esta prueba paramétrica; en su lugar, nos decantaremos por una prueba no parametríca de iguales características.
...
datos_fin <- data.frame(tiempos, alg)

kruskal.test(tiempos ~ alg, data = datos_fin)
```
Como el p-value es menor a 0.05, que es nuestro nivel de significancia establecido, podemos rechazar la hipótesis nula en favor de la alternativa; luego, para saber cuales de los algoritmos son diferentes podemos ocupar el procedimiento post-hoc a través de Holm.

```{r post-hoc_3}
pairwise.wilcox.test(datos_fin$tiempos, datos_fin$alg, p.adjust.method = "holm", paired = FALSE, exact = FALSE)
```

Con respecto al resultado del analisis post-hoc se puede asegurar con un 95% de confianza que existe una diferencia significativa entre los tiempos de A y B; sin embargo, entre A y C, y los B y C esto no sucede. 
...
```
En este caso las hipótesís son:
H0: Al comparar las mismas instancias de iguales características las las mejores soluciones encontradas son similares
H1: Al comparar las mismas instancias de iguales características las mejores soluciones encontradas tienen almenos una que es significativamente distinta
...
Con respecto al resultado del analisis post-hoc se puede asegurar con un 95% de confianza que, en las mismas instancias con iguales carácteristicas se puede apreciar una diferencia significativa entre los grupos A y B; sin embargo, entre los grupos A y C, y los B y C esto no sucede. 
